...
func Response(res http.ResponseWriter, resp responses.GeneralResponse) {
	if resp.Success == false {
		responseJSON(res, 200, H{"error": resp.Error, "success": false, "message": resp.Message})
	} else {
		responseJSON(res, 200, H{"data": resp.Data, "success": true, "message": resp.Message})
	}
}
...
func Response400(res http.ResponseWriter, resp responses.GeneralResponse) {
	if resp.Success == false {
		responseJSON(res, 400, H{"error": resp.Error, "success": false, "message": resp.Message})
	} else {
		responseJSON(res, 400, H{"data": resp.Data, "success": true, "message": resp.Message})
	}
}
